...
import list
--(changed ifThanElse to its file name on my computer, ite)
import ite
...
list_difference (l1) (h::t) = ite (member h l1)
                                  (list_difference (list_remove h l1) t)
                                  (list_difference l1 t)
...
set_forall p (mkSet l) = eql (length (filter p l)) (length l)
...
list_witness p (v::t) = ite (p v) (some v) (list_witness p t)
...
element_pair: a -> list b -> list (pair a b)
element_pair e nil = nil
element_pair e (h::t) = (mkPair e h)::(element_pair e t)

pair_list: list a -> list b -> list (pair a b)
pair_list nil l = nil
pair_list (h::t) l = (element_pair h l) ++ (pair_list t l)
...
set_product (mkSet la) (mkSet lb) = (mkSet (pair_list la lb))
...
set_powerset s = mkSet ((mkSet nil)::s::nil)
...
set_eql (mkSet nil) (mkSet nil) = true
set_eql (mkSet nil) s2 = false
set_eql s1 (mkSet nil) = false
set_eql (mkSet s1) (mkSet s2) = and (subset_element s1 s2) (subset_element s2 s1)
